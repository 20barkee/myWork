...
public class Magpie4 {
	...
	private String transformSmellsBadStatement(String statement) {
		statement = statement.trim();
		String lastChar = statement.substring(statement.length() - 1);
		if (lastChar.equals(".")) {
			statement = statement.substring(0, statement.length() - 1);
	...
	private String transformCantFind(String statement) {
		...
		int psnOfYou = findKeyword(statement, "I cannot find", 0);
		int psnOfMe = findKeyword(statement, "anywhere", psnOfYou + 13);

		String restOfStatement = statement.substring(psnOfYou + 13, psnOfMe).trim();
		return "I buried you " + restOfStatement + " in the backyard";
	}
